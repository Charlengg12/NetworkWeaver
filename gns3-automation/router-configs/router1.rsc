# MikroTik Router 1 Configuration Script
...
/ip address add address=192.168.100.10/24 interface=ether1 comment="Management Interface"
/ip address add address=10.1.1.1/24 interface=ether3 comment="LAN Interface"

# WAN Interface (DHCP Client)
/ip dhcp-client add interface=ether2 disabled=no comment="WAN Interface"

# DHCP Server for LAN
/ip pool add name=lan-pool ranges=10.1.1.100-10.1.1.200
/ip dhcp-server network add address=10.1.1.0/24 gateway=10.1.1.1 dns-server=8.8.8.8,8.8.4.4
...
# SNMP Configuration
/snmp set enabled=yes contact="admin@networkweaver.local" location="GNS3-Lab-Site1"
...
# /ip route add gateway=192.168.100.1 comment="Default route to management network"

# Save configuration
/system backup save name=router1-initial
