# MikroTik Router 2 Configuration Script
...
/ip address add address=192.168.100.11/24 interface=ether1 comment="Management Interface"
/ip address add address=10.2.1.1/24 interface=ether3 comment="LAN Interface"

# WAN Interface (DHCP Client)
/ip dhcp-client add interface=ether2 disabled=no comment="WAN Interface"

# DHCP Server for LAN
/ip pool add name=lan-pool ranges=10.2.1.100-10.2.1.200
/ip dhcp-server network add address=10.2.1.0/24 gateway=10.2.1.1 dns-server=8.8.8.8,8.8.4.4
...
# SNMP Configuration
/snmp set enabled=yes contact="admin@networkweaver.local" location="GNS3-Lab-Site2"
...
# Save configuration
/system backup save name=router2-initial
